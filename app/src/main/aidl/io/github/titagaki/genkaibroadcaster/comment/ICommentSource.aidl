// genkai-broadcaster (app/src/main/aidl/.../comment/) からのコピー。変更は配信アプリ側で行う。
// 提供側 (コメント取得アプリ) が Service として公開し、配信アプリが配信中だけ bind して使う。
// 検出用 action 名は genkai-broadcaster の docs/engineering/comment-overlay.md §3.1 を参照 (bind に権限は要らない)。
