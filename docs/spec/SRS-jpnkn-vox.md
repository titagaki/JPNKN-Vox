...
## 1. プロジェクト概要
外配信（IRL配信）において、配信者がコメントを「耳」と「目」の両方でリアルタイムに確認できる環境を提供するため、バックグラウンド動作に特化した読み上げ・表示アプリケーションを開発する。
コメントの取得先は jpnkn 掲示板・ツイキャス・Twitch に対応し、複数を同時に読み上げられる。
...
### 3.1 接続・通信機能
* **常時接続**: コメント取得先と常時接続し、リアルタイムでコメントを受信する。
  * jpnkn: MQTT で板のトピックを購読する。
  * ツイキャス: 配信が始まるまでポーリングし、始まったらコメントサーバへ WebSocket で接続する（`docs/spec/twicas-comment-spec.md`）。
  * Twitch: チャットの IRC へ匿名で接続し、チャンネルに JOIN する（`docs/spec/twitch-comment-spec.md`）。チャットは配信していない間も動くため、配信待ちの状態は持たない。
* **複数取得先**: 取得先を複数登録し、同時に読み上げられる。稼働中でも追加・削除でき、その場で接続・切断する。取得先ごとに識別色を持ち、どこから来たコメントかを見分けられる。
* **自動再接続**: 移動中のネットワーク切り替え（4G/5G/Wi-Fi）による切断時、指数バックオフアルゴリズム等を用いて自動復旧する。ツイキャスは枠の終了後も配信待ちに戻り、次の枠に自動で繋ぐ。Twitch は切断のたびに一定時間待って繋ぎ直す。
* **接続管理**: 取得先の保存、起動時の自動接続機能。
* **接続テスト**: 登録前に ID が正しいかを確かめられる。
...
* **ツイキャスの内部エンドポイント**: 公開ドキュメントの無いエンドポイントを使っているため、予告なく変わる可能性がある（公式 API v2 を使わない理由は `docs/spec/twicas-comment-spec.md` を参照）。
* **Twitch の IRC**: Twitch 自身が「レガシー」と位置づけている接続方法のため、将来止まる可能性がある（公式 API を使わない理由は `docs/spec/twitch-comment-spec.md` を参照）。